  ...
  return (
    <main>
      <div className='flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between'>
        <div>
          <h1 className='text-2xl font-semibold text-[var(--text)]'>{state.name}</h1>
          <div className='mt-3 flex flex-wrap gap-2 text-sm text-[var(--text-muted)]'>
            <span className='rounded-md border bg-[var(--surface)] px-2 py-1'>ID: {state.id}</span>
            {state.iso2 ? (
              <span className='rounded-md border bg-[var(--surface)] px-2 py-1'>Code: {state.iso2}</span>
            ) : null}
            {state.type ? (
              <span className='rounded-md border bg-[var(--surface)] px-2 py-1'>Type: {state.type}</span>
            ) : null}
            <span className='rounded-md border bg-[var(--surface)] px-2 py-1'>
              Country:{' '}
              <Link className='underline text-[var(--text)]' href={`/countries/${state.country_id}`}>
      ...
      <section className='mt-8 overflow-hidden rounded-xl border bg-[var(--surface)] shadow-sm'>
        <div className='border-b bg-[var(--surface-2)] px-4 py-3'>
          <div className='font-medium text-[var(--text)]'>Cities (first 50)</div>
        </div>
        ...
        <div className='border-t px-4 py-3 text-sm'>
          <Link className='underline text-[var(--text)]' href={`/cities?state_id=${state.id}`}>
            View all cities →
          </Link>
        </div>
      </section>
