      ...
      <div className='flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between'>
        <div>
          ...
          <div className='mt-3 flex flex-wrap gap-2 text-sm text-gray-700'>
            <span className='rounded-md border border-gray-200 bg-white px-2 py-1'>ID: {state.id}</span>
            {state.iso2 ? (
              <span className='rounded-md border border-gray-200 bg-white px-2 py-1'>Code: {state.iso2}</span>
            ) : null}
            {state.type ? (
              <span className='rounded-md border border-gray-200 bg-white px-2 py-1'>Type: {state.type}</span>
            ) : null}
            <span className='rounded-md border border-gray-200 bg-white px-2 py-1'>
              Country:{' '}
              <Link className='underline' href={`/countries/${state.country_id}`}>
