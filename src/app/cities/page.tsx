...
export const metadata: Metadata = {
  title: 'Cities',
  description: 'Browse cities with search, state/country filters, pagination, map previews, and city details.',
};
...
export default async function CitiesPage(props: { searchParams: Promise<SearchParams> }) {
  ...
  const json = await fetchJson(await internalApiUrl('/api/cities', apiQs.toString()));
  ...
  return (
    <main>
      <div className='flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between'>
        <div>
          <h1 className='text-2xl font-semibold text-(--text)'>Cities</h1>
          <p className='mt-1 text-sm text-(--text-muted)'>{json.meta.total} total · Filter by state_id / country_id</p>
        </div>
                        ...
                        >
                          <td className='px-5 py-4'>
                            <div className='flex items-center justify-between gap-4'>
                              <Link className='font-medium text-(--text) hover:underline' href={`/cities/${c.id}`}>
                                {c.name}
                              </Link>
                          ...
                          <td className='px-5 py-4 text-(--text-muted)'>
                            <Link className='underline text-(--text)' href={`/states/${c.state_id}`}>
                              {c.state_id}
                            </Link>
                          </td>
                          <td className='px-5 py-4 text-(--text-muted)'>
                            <Link className='underline text-(--text)' href={`/countries/${c.country_id}`}>
                              {c.country_id}
                            </Link>
                          </td>
                        </tr>
            ...
            <Card>
              ...
              <CardBody>
                <div className='text-sm text-(--text-muted)'>
                  Pick any city and click <span className='font-semibold text-(--text)'>Focus</span> to show a map
                  preview here.
                </div>
              </CardBody>
            </Card>
