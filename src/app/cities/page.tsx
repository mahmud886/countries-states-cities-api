    ...
    <main>
      <div className='flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between'>
        <div>
          ...
          <p className='mt-1 text-sm text-gray-700'>{json.meta.total} total · Filter by state_id / country_id</p>
        </div>
                ...
                json.data.map((c) => (
                  <tr key={c.id} className='border-t border-gray-100 hover:bg-gray-50'>
                    <td className='px-4 py-3'>
                      <Link className='font-medium hover:underline' href={`/cities/${c.id}`}>
                        {c.name}
                      </Link>
                    </td>
                    <td className='px-4 py-3 text-gray-700'>{c.type ?? '-'}</td>
                    <td className='px-4 py-3 text-gray-700'>
                      <Link className='underline' href={`/states/${c.state_id}`}>
                        {c.state_id}
                      </Link>
                    </td>
                    <td className='px-4 py-3 text-gray-700'>
                      <Link className='underline' href={`/countries/${c.country_id}`}>
                        {c.country_id}
                      </Link>
                    </td>
                  </tr>
