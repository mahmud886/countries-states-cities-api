  ...
  return (
    <main>
      <div className='flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between'>
        <div>
          <h1 className='text-2xl font-semibold text-[var(--text)]'>{city.name}</h1>
          <div className='mt-3 flex flex-wrap gap-2 text-sm text-[var(--text-muted)]'>
            <span className='rounded-md border bg-[var(--surface)] px-2 py-1'>ID: {city.id}</span>
            {city.type ? (
              <span className='rounded-md border bg-[var(--surface)] px-2 py-1'>Type: {city.type}</span>
            ) : null}
            <span className='rounded-md border bg-[var(--surface)] px-2 py-1'>
              State:{' '}
              <Link className='underline text-[var(--text)]' href={`/states/${city.state_id}`}>
                {city.state?.name ?? city.state_id}
              </Link>
            </span>
            <span className='rounded-md border bg-[var(--surface)] px-2 py-1'>
              Country:{' '}
              <Link className='underline text-[var(--text)]' href={`/countries/${city.country_id}`}>
