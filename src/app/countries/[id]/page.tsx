...
export default async function CountryDetailsPage(props: {
  params: Promise<{ id: string }>;
}) {
  const { id } = await props.params;

  const countryRes = await fetchJson(
    await internalApiUrl(`/api/countries/${id}`),
  );
  ...
  const statesRes = await fetchJson(
    await internalApiUrl(`/api/countries/${id}/states`, "limit=20&page=1"),
  );
  const citiesRes = await fetchJson(
    await internalApiUrl(`/api/countries/${id}/cities`, "limit=20&page=1"),
  );
  ...
  return (
    <main>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="mt-3 flex flex-wrap gap-2 text-sm text-[var(--text-muted)]">
            ...
            {country.region?.name ? (
              <span className="rounded-md border bg-[var(--surface)] px-2 py-1">
                ...
                <Link
                  ...
                  href={`/regions/${country.region.id}`}
                >
                  {country.region.name}
                </Link>
              </span>
            ...
            {country.subregion?.name ? (
              <span className="rounded-md border bg-[var(--surface)] px-2 py-1">
                ...
                <Link
                  className="underline text-[var(--text)]"
                  href={`/subregions/${country.subregion.id}`}
                >
                  {country.subregion.name}
                </Link>
              </span>
        ...
        <Link
          className="text-sm text-[var(--text)] underline"
          href="/countries"
        >
          Back
        </Link>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        <section className="overflow-hidden rounded-xl border bg-[var(--surface)] shadow-sm">
          ...
          <div className="border-t px-4 py-3 text-sm">
            <Link
              className="underline text-[var(--text)]"
              href={`/states?country_id=${country.id}`}
            >
              View all states →
            </Link>
          </div>
        ...
        <section className="overflow-hidden rounded-xl border bg-[var(--surface)] shadow-sm">
          ...
          <div className="border-t px-4 py-3 text-sm">
            <Link
              className="underline text-[var(--text)]"
              href={`/cities?country_id=${country.id}`}
            >
              View all cities →
            </Link>
          </div>
