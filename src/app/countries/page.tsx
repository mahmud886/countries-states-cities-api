...
export default async function CountriesPage(props: {
  ...
  const json = await fetchJson(
    await internalApiUrl("/api/countries", apiQs.toString()),
  );
  const focusedFromList = focusId
    ? json.data.find((c) => c.id === focusId)
    : null;

  const fetchCountry = async (id: number) => {
    const res = await fetch(await internalApiUrl(`/api/countries/${id}`), {
      cache: "no-store",
    });
  ...
  const focused =
    focusedFromList ?? (focusId ? await fetchCountry(focusId) : null);
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--text-muted)]">
            {json.meta.total} total · Search by name · Preview on map · Open
            details
          </p>
        </div>
