...
export async function generateMetadata(props: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  try {
    ...
    const name = (subregionRes.data?.name as string | undefined) ?? `Subregion ${id}`;
    return {
      title: name,
      description: `Subregion details for ${name}, including countries in the subregion.`,
    };
  } catch {
    return { title: `Subregion ${id}` };
  }
}
  ...
  return (
    <main>
      <div className='flex items-start justify-between'>
        <div>
          <h1 className='text-2xl font-semibold text-(--text)'>{subregion.name}</h1>
          <div className='mt-2 flex flex-wrap gap-2 text-sm text-(--text-muted)'>
            <span className='rounded-md border bg-(--surface) px-2 py-1'>ID: {subregion.id}</span>
            <span className='rounded-md border bg-(--surface) px-2 py-1'>
              Region:{' '}
              <Link className='underline text-(--text)' href={`/regions/${subregion.region_id}`}>
                {subregion.region?.name ?? subregion.region_id}
              </Link>
            </span>
          </div>
        </div>
        <Link className='text-sm underline text-(--text)' href={`/regions/${subregion.region_id}`}>
          Back
        </Link>
      </div>
