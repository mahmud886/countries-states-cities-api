      ...
      <div className='flex items-start justify-between'>
        <div>
          ...
          <div className='mt-2 flex flex-wrap gap-2 text-sm text-gray-700'>
            <span className='rounded-md border border-gray-200 bg-white px-2 py-1'>ID: {subregion.id}</span>
            <span className='rounded-md border border-gray-200 bg-white px-2 py-1'>
              Region:{' '}
              <Link className='underline' href={`/regions/${subregion.region_id}`}>
                {subregion.region?.name ?? subregion.region_id}
              </Link>
            </span>
          </div>
        </div>
        <Link className='text-sm underline' href={`/regions/${subregion.region_id}`}>
          Back
        </Link>
      </div>
          ...
          {countriesRes.data?.length ? (
            countriesRes.data.map((c: any) => (
              <li key={c.id} className='px-4 py-3'>
                <Link className='font-medium hover:underline' href={`/countries/${c.id}`}>
            ...
            <li className='px-4 py-10 text-center text-sm text-gray-600'>No countries</li>
          )}
        </ul>
      </section>
    </main>
