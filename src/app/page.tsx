...
async function fetchTotal(pathname: string) {
  const res = await fetch(await internalApiUrl(pathname, 'page=1&limit=1'), { cache: 'no-store' });
    ...
    <main>
      <div className='flex flex-col gap-6'>
        <div className='flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between'>
          <div>
            <div className='flex flex-wrap items-center gap-2'>
              <h1 className='text-2xl font-semibold text-[var(--text)]'>Dashboard Overview</h1>
        ...
        <div className='grid gap-4 sm:grid-cols-2 lg:grid-cols-5'>
          <StatCard label='Countries' value={countries ?? '—'} accent='primary' />
          ...
          <StatCard label='Subregions' value={subregions ?? '—'} accent='neutral' />
        </div>

        <div className='grid gap-6 lg:grid-cols-12'>
          <div className='lg:col-span-8'>
            <MapEmbed title='Global map' lat={20} lng={0} zoom={2} heightClassName='h-[360px]' />
          </div>
          <div className='flex flex-col gap-6 lg:col-span-4'>
            <Card>
              <CardHeader
                ...
                right={
                  <Link className='text-sm underline text-[var(--text)]' href='/docs'>
                    Docs →
                  </Link>
                }
              />
            ...
            <Card>
              <CardHeader title='API hints' description='Query options supported by list endpoints' />
              <CardBody>
