...
export const metadata: Metadata = {
  title: 'Regions',
  description: 'Browse regions with search, pagination, and map previews. Open region pages to explore subregions.',
};
...
export default async function RegionsPage(props: { searchParams: Promise<SearchParams> }) {
  ...
  const json = await fetchJson(await internalApiUrl('/api/regions', apiQs.toString()));
  ...
  return (
    <main>
      <div className='flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between'>
        <div>
          <h1 className='text-2xl font-semibold text-(--text)'>Regions & Subregions</h1>
          <p className='mt-1 text-sm text-(--text-muted)'>{json.meta.total} total · Focus a region to preview</p>
        </div>
                        ...
                        >
                          <td className='px-5 py-4'>
                            <div className='flex items-center justify-between gap-4'>
                              <Link className='font-medium text-(--text) hover:underline' href={`/regions/${r.id}`}>
                                {r.name}
                              </Link>
                              <Link className='text-xs underline text-(--text)' href={focusHref(r.id)}>
                          ...
                          <td className='px-5 py-4 text-(--text-muted)'>
                            <Link className='underline text-(--text)' href={`/regions/${r.id}`}>
                              Open →
                            </Link>
                          </td>
                          <td className='px-5 py-4 text-right font-mono text-xs text-(--text-muted)'>{r.id}</td>
                        </tr>
                      );
                    })
                  ) : (
                    <tr className='border-t'>
                      <td className='px-5 py-12 text-center text-sm text-(--text-muted)' colSpan={3}>
                        No regions found.
                      </td>
                    </tr>
                  )}
            ...
            <Card>
              ...
              <CardBody>
                <div className='text-sm text-(--text-muted)'>
                  Pick any region and click <span className='font-semibold text-(--text)'>Focus</span> to show a map
                  preview here.
                </div>
              </CardBody>
            </Card>
              ...
              <CardBody className='space-y-2 text-sm'>
                {'subregions' in focused && focused.subregions?.length ? (
                  focused.subregions.slice(0, 12).map((s) => (
                    <Link key={s.id} className='block text-(--text) underline' href={`/subregions/${s.id}`}>
                      {s.name}
                    </Link>
                  ))
                ) : (
                  <div className='text-(--text-muted)'>Open the region details page to see subregions.</div>
                )}
                <div className='pt-2'>
