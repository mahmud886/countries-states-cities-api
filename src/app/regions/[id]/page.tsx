  ...
  return (
    <main>
      <div className='flex items-start justify-between'>
        <div>
          <h1 className='text-2xl font-semibold text-[var(--text)]'>{region.name}</h1>
          <div className='mt-2 text-sm text-[var(--text-muted)]'>Region ID: {region.id}</div>
        </div>
            ...
            {region.subregions?.length ? (
              region.subregions.map((s) => (
                <li key={s.id} className='px-4 py-3'>
                  <Link className='text-[var(--text)] hover:underline' href={`/subregions/${s.id}`}>
                    {s.name}
                  </Link>
                </li>
              ))
            ) : (
              <li className='px-4 py-10 text-center text-sm text-[var(--text-muted)]'>No subregions</li>
            )}
          </ul>
        </section>
