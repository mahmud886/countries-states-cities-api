    ...
    <main>
      <div className='flex items-start justify-between'>
        <div>
          ...
          <div className='mt-2 text-sm text-gray-700'>Region ID: {region.id}</div>
        </div>
            ...
            {region.subregions?.length ? (
              region.subregions.map((s) => (
                <li key={s.id} className='px-4 py-3'>
                  <Link className='hover:underline' href={`/subregions/${s.id}`}>
                    {s.name}
                  </Link>
                </li>
              ))
            ) : (
              <li className='px-4 py-10 text-center text-sm text-gray-600'>No subregions</li>
            )}
          </ul>
        </section>
