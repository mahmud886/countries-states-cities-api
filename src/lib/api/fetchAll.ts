export async function fetchAllPages<T>(opts: {
  ...
  while (true) {
    const { data, error, count } = await opts.fetchPage(from, from + chunkSize - 1);
    if (error) return { data: null as T[] | null, error, total: null as number | null };
  ...
  return { data: all, error: null as { message: string } | null, total: total ?? all.length };
}
